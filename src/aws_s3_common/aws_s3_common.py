...
def bucket_stats(bucket="", metric="Maximum", profile="", region="us-east-1", resource=None):
    ...
    s3r = resource if resource else boto3.resource('s3')
    if bucket:
...
def prefix_stats(bucket, prefix="", resource=None):
...
def get_objects(bucket, prefix="", full=False, resource=None):
...
def get_prefixes(bucket, prefix="", full=False, client=None):
    ''' Return a list ob prefixes in a bucket and optional prefix
        ...
        Returns:
          List of prefixes
    '''
    s3c = client if client else boto3.client('s3')
